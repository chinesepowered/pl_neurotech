...
A neural data wallet + AI agent marketplace on Filecoin. Users record EEG sessions, store on Filecoin, set consent/pricing, list on a marketplace, and autonomous AI agents (Cerebras llama-3.3-70b) can browse/evaluate/purchase datasets via streaming tool calls.

## Current State

**All pages implemented and building cleanly.** The project compiles with zero type errors on `pnpm build`. Every page renders. The smart contract is written but needs deployment (requires tFIL in wallet).

### What Works
- Landing page with animations
- EEG recording with live 8-channel Canvas waveforms (synthetic data)
- Full record → consent → upload → list flow
- Marketplace browse and detail pages with demo data
- AI agent page with Cerebras streaming and tool call display
- Dashboard with charts, transactions, activity feed
- All 7 API routes
- Smart contract + deploy/seed scripts

### What Needs External Setup
- `FILECOIN_PRIVATE_KEY` — needs a funded Calibration testnet wallet
...
### Demo Mode
All API routes gracefully fallback to demo mode when contract/keys aren't configured. The marketplace shows 5 hardcoded demo datasets, uploads generate fake CIDs, purchases return mock tx hashes. The app is fully navigable without any external setup — only the AI agent requires the Cerebras API key.
...
- Pink noise background via Voss-McCartney algorithm
- Artifacts: eye blinks on Fp1/Fp2, muscle bursts on T3/T4
- Canvas ring buffer rendering at ~60fps
